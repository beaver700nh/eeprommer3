...
uint16_t TftCalc::t_center_x_P(uint16_t box, const char *text, uint8_t size) {
  return (box - t_width_P(text, size)) / 2;
}
...
uint16_t TftCalc::t_center_x_P(TftCtrl &tft, const char *text, uint8_t size) {
...
uint16_t TftCalc::t_width_P(const char *text, uint8_t size) {
