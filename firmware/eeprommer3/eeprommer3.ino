...
SdCtrl::Status initialize() {
  ...
  auto *x = (uint8_t *) malloc(sizeof(uint8_t));
  *x = 3;
  SER_DEBUG_PRINT(*x, 'd');
  SER_DEBUG_PRINT(x, 'p');
  ...
  *y = 5;
  ...
  public:
    ...
    void test() {
      SER_DEBUG_PRINT(this->x, 'd');
    }
  private:
    int x;
  };

  auto *z = new testfoo(-10);
  z->test();
  ...
  ee.get_io_exp(1)->begin(0x20);
  ee.get_io_exp(1)->set_iodir(MCP_EE_WE_PORT, OUTPUT);
  ee.get_io_exp(1)->write_port(MCP_EE_WE_PORT, 0xFF);
